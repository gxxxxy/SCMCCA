function varargout = rtr_multi_stiefel(fns,params)
...
while stop_outer==0,
    ...
    %compute the multiplier lambda
    lambda=cell(mm,1);AX=cell(mm,1);
    for i=1:mm
        AX{i}=0;
        for j=1:mm
            AX{i}=AX{i}+params.A{i,j}*x{j};
        end
        tmp=x{i}'*AX{i};lambda{i}=(tmp+tmp')/2;
    end
    
    % solve TR subproblem
    [eta,numit,stop_inner] = tCG(fns,x,fgradx,eta,Delta,theta,kappa,min_inner,max_inner,useRand,debug,lambda,mm);
    ...
    Heta = fns.fhess_new(x,eta,lambda);
    ...
    if debug > 1,
        ...
        actrho = (fns.f(x) - fns.f(x_prop)) / (m(x,Zero) - m(x,eta));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%% truncated CG %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [eta,inner_it,stop_tCG] = tCG(fns,x,grad,eta,Delta,theta,kappa,min_inner,max_inner,useRand,debug,lambda,mm);
...
for j = 1:max_inner,
    ...
    Hxd = fns.fhess_new(x,delta,lambda);
...
for i=1:20,
    ...
    HT = fns.fhess(x,T);
    PHT = fns.proj(x,HT);
    HTPHT=cell(length(x),1);
    for j=1:length(x)
        HTPHT{j}=HT{j}-PHT{j};
    end
    errnrm = fns.g(x,HTPHT,HTPHT);
    errnrm = sqrt(errnrm);
...
for i=1:5,
    eta = fns.randT(x);
    tmp=sqrt(fns.g(x,eta,eta));
    for j=1:length(x)
        eta{j} = eta{j} / tmp;
    end
    for t = 10.^[0:-1:-6],
        teta=cell(length(x),1);
        for j=1:length(x)
            teta{j} = t*eta{j};
        end
        fxe = fns.f( fns.R(x,teta));
